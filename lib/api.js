...
/*!
 * Api
 * Attaches the Statique methods to self
 *
 * @name Api
 * @function
...
var Api = module.exports = function () {
    ...
    self.setRoutes = function (routes) {
        ...
        Object.keys(routes).forEach(function (k) {
            ...
            if (cRoute.re.constructor.name !== "RegExp") {
                cRoute.re = RegexParser(typeof cRoute.re === "string" ? cRoute.re : r);
            }
            self.regexpRoutes.push(cRoute);
            delete routes[r];
        });
    ...
    self.getRoute = function (url) {
        ...
        var routeObj = self.routes[url] || self.routes[url + "/"] || null;
        route.url = (routeObj || {}).url || routeObj;
        route.reqUrl = route.url || url;
        route.data = routeObj;

        // Handle regexp routes
        for (var i = 0, cRegexpRoute; i < self.regexpRoutes.length; ++i) {
            cRegexpRoute = self.regexpRoutes[i];
    ...
    self.readFile = function (file, callback) {
        ...
        var stream = Fs.createReadStream(self.root + file)
          , content = ""
          , error = ""
          ;
    ...
    self.error = function (req, res, errCode, errMessage) {
        ...
        var configErrors = Object(self.errors)
