...
function Statique(options) {
    ...
    self.serve = self.serve.bind(self)
...
Statique.prototype.sendRes = function (res, statusCode, mimeType, content, otherHeaders) {
    ...
    var self = this
      , lMimeType = Mime.getType(String(mimeType))
      ;

    if (Mime.getExtension(lMimeType) === mimeType) {
        mimeType = lMimeType;
    }
...
Statique.prototype.serveFile = function (path, statusCode, res, req, additionalHeaders, customRoot) {
    ...
    var activeRoot = Path.normalize(customRoot || self.root)
      , activePath = Path.normalize(path.url || path.reqUrl)
      , fullPath = Path.normalize(activeRoot + activePath)
      , relative = Path.relative(activeRoot, fullPath)
      , stats = null
      ;
    ...
    var mtime = Date.parse(stats.mtime)
      ...
      , contentType = Mime.getType(path.reqUrl)
      , headers = {
