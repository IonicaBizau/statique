...
var Statique = module.exports = function (options) {
    ...
    var apiFunctions = Object.keys(Statique.prototype);
    apiFunctions.forEach(function (c) {
        self[c] = that[c] = that[c].bind(that);
    });
...
Statique.prototype.setRoutes = function (routes) {
    ...
    routes = self.routes = Ul.merge(routes, self.routes);
    self.regexRoutes = [];
    ...
    Object.keys(routes).forEach(function (k) {
        cRoute = routes[k];
        if (!cRoute.re) { return; }
        if (cRoute.re.constructor.name !== "RegExp") {
            cRoute.re = RegexParser(
                typeof cRoute.re === "string" ? cRoute.re : k
            );
        }
        self.regexRoutes.push(cRoute);
        delete routes[k];
    });
...
Statique.prototype.getRoute = function (url) {
    ...
    var routeObj = this.routes[url] || this.routes[url + "/"] || null;
    route.url = (routeObj || {}).url || routeObj;
    route.reqUrl = route.url || url;
    route.data = routeObj;

    // Handle regex routes
    for (var i = 0, cRegexRoute; i < this.regexRoutes.length; ++i) {
        cRegexRoute = this.regexRoutes[i];
        if (cRegexRoute.re.test(url)) {
            for (var k in cRegexRoute) {
                route[k] = cRegexRoute[k];
            }
            break;
        }
    }
    ...
    if (routeObj && typeof routeObj.handler === "function") {
        route.handler = routeObj.handler;
    }
...
Statique.prototype.readFile = function (file, callback) {
    ...
    var stream = Fs.createReadStream(this.root + file)
      , content = ""
      , error = ""
      ;
...
Statique.prototype.serveRoute = function (route, req, res) {
    ...
    if (routeToServe.url && typeof routeToServe.url[method] === "function") {
        d.run(function() {
            routeToServe.url[method](req, res, form.emitter);
        });
        return self;
    }
...
Statique.prototype.error = function (req, res, errCode, errMessage) {
    ...
    var configErrors = Object(this.errors)
      , errPage = configErrors[errCode]
      ;

    if (errPage) {
        return this.serveFile(errPage, errCode, res, req, {
            "cache-control": "no-cache"
        });
    }

    return this.sendRes(res, errCode, "text", errMessage);
};
